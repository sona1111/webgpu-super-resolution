...
fn main(@builtin(global_invocation_id) global_id: vec3u) {
    ...
    for (var z = 0u; z < u32(ufs.inputSizes[0]); z = z + 1u) {
        ...
        if(y - 1 >= 0 && x - 1 >= 0){
            ...
            image_patch[0] = inputImage.numbers[ufs.inputOffset + imageIndex];
        }
        image_patch[1] = 0.;
        if(y - 1 >= 0){
            imageIndex = z * image_area + u32(y+-1) * u32(ufs.inputSizes[1]) + u32(x);
            image_patch[1] = inputImage.numbers[ufs.inputOffset + imageIndex];
        }
        image_patch[2] = 0.;
        if(y - 1 >= 0 && x + 1 < ufs.outputSizes[0]){
            imageIndex = z * image_area + u32(y+-1) * u32(ufs.inputSizes[1]) + u32(x + 1);
            image_patch[2] = inputImage.numbers[ufs.inputOffset + imageIndex];
        }
        image_patch[3] = 0.;
        if(x - 1 >= 0){
            imageIndex = z * image_area + u32(y) * u32(ufs.inputSizes[1]) + u32(x+-1);
            image_patch[3] = inputImage.numbers[ufs.inputOffset + imageIndex];
        }
        imageIndex = z * image_area + u32(y) * u32(ufs.inputSizes[1]) + u32(x);
        image_patch[4] = inputImage.numbers[ufs.inputOffset + imageIndex];
        image_patch[5] = 0.;
        if(x + 1 < ufs.outputSizes[0]){
            imageIndex = z * image_area + u32(y) * u32(ufs.inputSizes[1]) + u32(x + 1);
            image_patch[5] = inputImage.numbers[ufs.inputOffset + imageIndex];
        }
        image_patch[6] = 0.;
        if(y + 1 < ufs.outputSizes[1] && x - 1 >= 0){
            imageIndex = z * image_area + u32(y+1) * u32(ufs.inputSizes[1]) + u32(x+-1);
            image_patch[6] = inputImage.numbers[ufs.inputOffset + imageIndex];
        }
        image_patch[7] = 0.;
        if(y + 1 < ufs.outputSizes[1]){
            imageIndex = z * image_area + u32(y+1) * u32(ufs.inputSizes[1]) + u32(x);
            image_patch[7] = inputImage.numbers[ufs.inputOffset + imageIndex];
        }
        image_patch[8] = 0.;
        if(y + 1 < ufs.outputSizes[1] && x + 1 < ufs.outputSizes[0]){
            imageIndex = z * image_area + u32(y+1) * u32(ufs.inputSizes[1]) + u32(x + 1);
            image_patch[8] = inputImage.numbers[ufs.inputOffset + imageIndex];
        }
